...
public interface JobTagMappingRepository extends JpaRepository<JobTagMapping, Integer> {
    @Query("SELECT DISTINCT j.recruitment FROM JobTagMapping j WHERE j.tag.id IN :tagIds and j.status = 0 and j.recruitment.status = 0 and j.tag.status = 0 and j.recruitment.permissionStatus = 0 GROUP BY j.recruitment.recruitmentId HAVING COUNT(DISTINCT j.tag.id) = :tagCount")
